...
### 4. 2310-轮位压缩筛 (Wheel-2310 Bitwise Sieve)

**目录：** `algorithms/wheel-2310-sieve/`

在 210-轮筛基础上再引入质数 11，以 **2310 = 2×3×5×7×11** 为轮模数。φ(2310) = 480，每 2310 个整数中仅约 20.8% 为候选数，比 210-轮的 22.9% 再减少约 9%。480 个候选位压缩存储于 8 个 64 位整数中；n = 1,000,000 时筛数组约 **27 KB**，可完全驻留 L1 缓存。内层标记循环使用增量模运算跟踪复合数余数，无需额外的大型乘积表。

- 时间复杂度：O(n log log n)
- 空间复杂度：O(n / 2310 × 480 位) ≈ O(n / 38.5)
- 适用场景：批量生成某范围内所有质数，缓存友好型实现
